...
def mostrar_encabezado():
    ...
    print("|||| LAST.FM TOP ARTISTS RAW DOWNLOADER ||||".center(ANCHO))
...
def descargar_top_artists_raw(usuario):
    ...
    archivo_json = (
        f"{ruta_usuario}/top_artists_raw_{timestamp}.json"
    )
    ...
    while pagina <= total_paginas:
        ...
        if pagina == 1:
            ...
            total_paginas = int(
                response["topartists"]["@attr"]["totalPages"]
            )
            ...
            print(
                f"Total de artistas RAW: "
                f"{formato_numero(total_artistas)} "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Descarga RAW de top artistas desde Last.fm"
    )
    ...
    print(
        ...
        + f"Descargando top artistas RAW de {args.usuario}..."
        + Style.RESET_ALL
    )

    archivo = descargar_top_artists_raw(args.usuario)
