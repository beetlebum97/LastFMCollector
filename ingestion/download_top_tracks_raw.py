...
def mostrar_encabezado():
    ...
    print("|||| LAST.FM TOP TRACKS RAW DOWNLOADER ||||".center(ANCHO))
...
def descargar_top_tracks_raw(usuario):
    ...
    archivo_json = (
        f"{ruta_usuario}/top_tracks_raw_{timestamp}.json"
    )
    ...
    while pagina <= total_paginas:
        ...
        if pagina == 1:
            ...
            total_paginas = int(
                response["toptracks"]["@attr"]["totalPages"]
            )

            total_tracks = int(
                response["toptracks"]["@attr"]["total"]
            )

            print()

            print(
                f"Total de canciones RAW: "
                f"{formato_numero(total_tracks)} "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Descarga RAW de top canciones desde Last.fm"
    )
    ...
    print(
        ...
        + f"Descargando top canciones RAW de {args.usuario}..."
        + Style.RESET_ALL
    )

    archivo = descargar_top_tracks_raw(args.usuario)
