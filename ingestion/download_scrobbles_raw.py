...
def mostrar_encabezado():
    ...
    print("|||| LAST.FM RAW SCROBBLES DOWNLOADER ||||".center(ANCHO))
...
def descargar_scrobbles_raw(usuario):
    ...
    os.makedirs(f"{ruta_raw}/{usuario}", exist_ok=True)
    
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

    archivo_json = (
        f"{ruta_raw}/{usuario}/scrobbles_raw_{timestamp}.json"
    )
    ...
    while pagina <= total_paginas:
        ...
        if pagina == 1:
            ...
            print(
                f"Total de scrobbles RAW: "
                f"{formato_numero(total_scrobbles)} "
        ...
        json.dump(
            {
                ...
                "origen": "Last.fm API",
                "metodo": "user.getRecentTracks",
                "total_paginas": total_paginas,
                "total_scrobbles": len(tracks_raw),
                "tracks": tracks_raw
            },
            json_file,
            ensure_ascii=False,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Descarga RAW de scrobbles desde Last.fm"
    )
    ...
    print(
        ...
        + f"Descargando scrobbles RAW de {args.usuario}..."
        + Style.RESET_ALL
    )

    total = descargar_scrobbles_raw(args.usuario)

    tiempo_total = time.time() - tiempo_inicio

    print(
        Fore.LIGHTGREEN_EX
        + f"\n✓ Scrobbles RAW descargados: {formato_numero(total)}"
        + Style.RESET_ALL
    )
