...
def mostrar_encabezado():
    ...
    print("|||| LAST.FM TOP TRACKS STAGING PROCESSOR ||||".center(ANCHO))
...
def obtener_ultimo_raw(usuario):
    ...
    archivos = [
        ...
        if archivo.startswith("top_tracks_raw_")
    ...
    if not archivos:
        ...
        raise FileNotFoundError(
            "No se encontraron archivos RAW de canciones"
        )
...
def procesar_tracks(raw_data):

    tracks_staging = []

    for pagina in raw_data["paginas"]:

        tracks = pagina["toptracks"]["track"]
        ...
        for track in tracks:
            ...
            tracks_staging.append(track_staging)

    return tracks_staging


def guardar_json(tracks, archivo_json):
...
def guardar_csv(tracks, archivo_csv):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Procesa top canciones RAW a STAGING"
    )
    ...
    print(
        ...
        + f"Procesando top canciones RAW de {args.usuario}..."
        + Style.RESET_ALL
    )
    ...
    tracks_staging = procesar_tracks(raw_data)
    ...
    archivo_json = (
        f"{ruta_usuario_staging}/top_tracks_staging.json"
    )

    archivo_csv = (
        f"{ruta_usuario_staging}/top_tracks_staging.csv"
    )
    ...
    print(
        ...
        + formato_numero(len(tracks_staging))
        + Style.RESET_ALL
    )
