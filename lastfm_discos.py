...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Uso: python lastfm_discos.py <nombre_usuario>")
        sys.exit(1)
    ...
    print(f"Usuario verificado. Obteniendo discos reproducidos por {usuario}...")
    ...
    try:
        ...
        contador = procesar_discos(usuario, archivo_salida, archivo_csv)
        
        # Resultado con formato de separación de miles
        print(f"-> Discos registrados: {formato_numero(contador)}")
...
def procesar_discos(usuario, archivo_salida, archivo_csv):
    url = "http://ws.audioscrobbler.com/2.0/"
    ...
    with open(archivo_salida, 'w', encoding='utf-8') as txt_file, \
        ...
        csv_writer.writerow(['Puesto', 'Disco', 'Artista', 'Scrobbles'])  # Cabecera
