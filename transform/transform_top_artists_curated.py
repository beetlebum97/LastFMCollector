...
def mostrar_encabezado():
    ...
    print("|||| LAST.FM TOP ARTISTS CURATED PROCESSOR ||||".center(ANCHO))
...
def obtener_staging(usuario):
    ...
    archivo = (
        f"{ruta_staging}/{usuario}/top_artists_staging.json"
    )
...
def procesar_curated(artistas_staging):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Procesa artistas STAGING a CURATED"
    )
    ...
    print(
        ...
        + f"Procesando artistas STAGING de "
        + f"{args.usuario}..."
        + Style.RESET_ALL
    )
    ...
    with open(
        ...
        artistas_staging = json.load(
            staging_file
        )

    artistas_curated = procesar_curated(
        artistas_staging
    )
    ...
    archivo_json = (
        ...
        "/top_artists_curated.json"
    )

    archivo_csv = (
        f"{ruta_usuario_curated}"
        "/top_artists_curated.csv"
    )
